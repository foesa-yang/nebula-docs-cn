...
!!! enterpriseonly
     ...
     当前仅企业版使用 Basic 角色权限，支持 Tag/Edge Type 的权限控制。
...
- 只允许用户在指定图空间后对 Basic 角色进行授权（GRANT）和撤销权限（REVOKE）操作，不允许授权给其他角色用户。
...
# 授予 test 用户读写 Tag 为 `player` 和 Edge Type 为 `follow` 和 `serve` 的权限 
# 授予用户指定点、边读写权限必须在指定图空间后
...
!!! caution
    ...
    对于 Basic 角色用户，使用语句查询时指定了无权限的 Tag/Edge Type 将报错。在遍历过程中，只访问有权限的 Tag/Edge Type。
