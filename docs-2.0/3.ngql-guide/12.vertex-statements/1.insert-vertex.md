...
- `tag_name`：点关联的 Tag（点类型）。Tag 的创建，详情请参见 [CREATE TAG](../10.tag-statements/1.create-tag.md)。
  ...
  !!! caution
        ...
        NebulaGraph {{ nebula.release }} 中默认不支持插入无 Tag 的点。如需使用无 Tag 的点要在集群内所有 Graph 服务的配置（`nebula-graphd.conf`）中新增`--graph_use_vertex_key=true`；在所有 Storage 服务的配置（`nebula-storaged.conf`）中新增`--use_vertex_key=true`。命令示例如`INSERT VERTEX VALUES "1":();`。
