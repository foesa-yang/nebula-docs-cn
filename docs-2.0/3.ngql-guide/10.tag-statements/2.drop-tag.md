...
!!! Compatibility
    ...
    在 NebulaGraph 3.x 版本中，允许存在无 Tag 的点，但在 NebulaGraph {{nebula.release}} 中默认不支持无 Tag 的点。如需使用无 Tag 的点要在集群内所有 Graph 服务的配置（`nebula-graphd.conf`）中新增`--graph_use_vertex_key=true`；在所有 Storage 服务的配置（`nebula-storaged.conf`）中新增`--use_vertex_key=true`。
